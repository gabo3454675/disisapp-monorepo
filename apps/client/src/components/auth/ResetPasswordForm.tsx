...
interface ResetPasswordFormProps {
  ...
  initialEmail?: string;
  /** Si viene del perfil, tras éxito puede redirigir a login en vez de auto-login */
  fromProfile?: boolean;
}
...
export function ResetPasswordForm({ initialEmail, fromProfile }: ResetPasswordFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (fromProfile) {
        // Usuario logueado cambiando desde perfil: limpiar sesión y redirigir a login con mensaje
        clearAuth();
        router.push('/login?success=password_changed');
      } else {
        // Usuario no logueado (clave temporal): auto-login con el nuevo JWT
